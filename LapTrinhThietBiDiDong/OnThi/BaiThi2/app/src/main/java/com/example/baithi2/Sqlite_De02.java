...
public class Sqlite_De02 extends SQLiteOpenHelper {
    ...
    public ArrayList<HoaDon_3107> getContactBySearch(String s){
        if(s.equals("")){
            ArrayList<HoaDon_3107> list = getAllContact();
            return list;
        }
        int money = Integer.parseInt(s);
        ArrayList<HoaDon_3107> list = new ArrayList<>();
        // câu truy vấn
        String sql = "Select * from "+TableName +" Where donGia*soNgay > "+money+" Order By soPhong DESC";
        //Lấy đối tượng csdl sqlite
        SQLiteDatabase db = this.getReadableDatabase();
        //Chạy câu truy vấn trả về dạng Cursor
        Cursor cursor = db.rawQuery(sql, null);
